"""
Modèle de données SQLite pour PlacardCAD.
Gestion des projets et aménagements.
"""
...
# Paramètres par défaut pour un nouvel aménagement
PARAMS_DEFAUT = {
    "hauteur": 2500,
    "largeur": 3000,
    "profondeur": 600,
...
CREATE TABLE IF NOT EXISTS amenagements (
    ...
    nom             TEXT NOT NULL DEFAULT 'Aménagement 1',
...
class Database:
    """Gestionnaire de base de données SQLite pour PlacardCAD."""
    ...
    def supprimer_projet(self, projet_id: int):
        """Supprime un projet et ses aménagements (CASCADE)."""
    ...
    def lister_projets(self) -> list[dict]:
        """Retourne tous les projets triés par date de modification."""
    ...
    def creer_amenagement(self, projet_id: int, nom: str = "",
        ...
        """Crée un aménagement et retourne son id."""
        # Numéro auto
        row = self.conn.execute(
    ...
    def modifier_amenagement(self, amenagement_id: int, **kwargs):
        """Modifie les champs d'un aménagement."""
        ...
        # Mettre à jour la date du projet parent
        row = self.conn.execute(
    ...
    def get_amenagement(self, amenagement_id: int) -> Optional[dict]:
        """Retourne un aménagement par son id."""
        row = self.conn.execute(
    ...
    def lister_amenagements(self, projet_id: int) -> list[dict]:
        """Retourne les aménagements d'un projet triés par numéro."""
        rows = self.conn.execute(
    ...
    def get_params(self, amenagement_id: int) -> dict:
        """Retourne les paramètres d'un aménagement en tant que dict."""
        row = self.conn.execute(
